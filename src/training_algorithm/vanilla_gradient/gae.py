...
from src.training_algorithm.vanilla_gradient.critic_interface import Critic
...
class GAE(Critic):  # Why isnt this camelcase
    ...
    def train(self, rewards, actions, states, nxt_states):
        ...
        non_final_mask = torch.tensor(
            tuple(torch.any(torch.logical_not(torch.isnan(s))) for s in nxt_states),
            device=self.device,
            dtype=torch.bool,
        )
        non_final_next_states = [
            s for s in nxt_states if torch.any(torch.logical_not(torch.isnan(s)))
        ]
        non_final_next_states = (
        ...
        while diff > self.eps:
            ...
            self.optimizer.step()
            new_norm = torch.linalg.norm(
                torch.cat([param.flatten() for param in self.policy.parameters()])
            )
