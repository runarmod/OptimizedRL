        ...
        try:
            ...
            if (
                ...
                for var_name, binfo in node_bounds.items():
                    plb, pub = self.parent_bounds[parent_num].get(
                        var_name, (None, None)
                    )
            ...
            self.parent_bounds[node_num] = {
                v: (binfo.get("lb"), binfo.get("ub"))
                for v, binfo in node_bounds.items()
            }
        except Exception:
            pass
...
class SCIPSolver:
    ...
    def solve(self, init_node):
        ...
        try:
            ...
            model.setObjective(
                quicksum(float(c[i]) * vars_[i] for i in range(n_vars)),
                sense="minimize",
            )
            ...
            if root_lp is not None and _append_unique(root_lp):
                added_root_lp = 1
            ...
            for node_num, info in tracker.node_map.items():
                ...
                ineqlin = _ordered_dual_array(
                    ineq_cons_names, info.get("duals_ineq", {})
                )
            ...
            if len(results) < min_pool_size:
                ...
                if ref_x is not None:
                    for conds in self._pseudo_branch_conds(
                        init_node, ref_x, max_vars=8
                    ):
                        if len(results) >= min_pool_size:
                            break
                        lp_alt = self._solve_lp_with_conds(init_node, conds)
