    ...
    def solve(self, max_iter=10000, store_pool=False, verbose=False):
        ...
        while len(self.queue) > 0 and iter <= max_iter:
            ...
            for neighbor in self.generate_neighbors(
                x_fixed, self.integer, self.init_node["bounds"]
            ):
...
def translate_to_highspy(c, A_ub=None, b_ub=None, A_eq=None, b_eq=None, bounds=None):
    ...
    if A_eq is not None and b_eq is not None:
        ...
        assert num_vars_eq == num_vars, (
            "A_eq must have the same number of columns as variables"
        )
        for i in range(num_eq_constraints):
            coefficients = A_eq[i, :]
            # Equality constraint: b_eq[i] <= A_eq[i] * x <= b_eq[i]
            h.addRow(
                b_eq[i], b_eq[i], num_vars, list(range(num_vars)), coefficients.tolist()
            )

    # Add inequality constraints (A_ub x <= b_ub)
    if A_ub is not None and b_ub is not None:
        num_ub_constraints, num_vars_ub = A_ub.shape
        assert num_vars_ub == num_vars, (
            "A_ub must have the same number of columns as variables"
        )
...
def optimize_node2(c, A_ub, b_ub, bounds):
    return scipy.optimize.linprog(c, A_ub, b_ub, None, None, bounds)
...
def optimize_node4(i):
    bound = bounds[i]
    h = translate_to_highspy(c, A_ub, b_ub, A_eq, b_eq, bound)
...
def bruteForceSolveMILP(
    node, max_iter=10000, store_pool=False, verbose=False, processes=None
):
...
class BruteForcePara(Solver):
    ...
    def translate_to_highspy(
        self, c, A_ub=None, b_ub=None, A_eq=None, b_eq=None, bounds=None
    ):
        """
        Translate a linear programming problem defined in scipy format to highspy.
        Chatgpt wrote this
    ...
    def optimize_node_process(self, c, A_ub, b_ub, A_eq, b_eq, bound):
        ...
        h = self.translate_to_highspy(c, A_ub, b_ub, A_eq, b_eq, bound)
        h.setOptionValue("log_to_console", False)
        h.silent()
        h.run()
    ...
    def solve(self, node, max_iter=10000):
        ...
        while len(queue) > 0 and iter <= max_iter:
            ...
            pool.append(node)
            for neighbor in self.generate_neighbors(
                x_fixed, integer, orig_node["bounds"]
            ):
                neighbor_tuple = tuple(neighbor)
                if neighbor_tuple not in visited:
                    queue.append(neighbor)
